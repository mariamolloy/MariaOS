...
(function (TSOS) {
    var Cpu = /** @class */ (function () {
        ...
        Cpu.prototype.cycle = function () {
            ...
            switch (opCode) {
                case "A9": //load the accumulator with a constant
                    this.Acc = parseInt(_MemoryAccessor.read(this.getPhysicalAddress(this.PC + 1, _CurrentPartition)), 16);
                    this.PC = this.PC + 2;
                    break;
                case "AD": //Load the accumulator from memory
                    this.Acc = parseInt(_MemoryAccessor.read(this.getPhysicalAddress(this.lilEndianTranslator(), _CurrentPartition)), 16);
                    this.PC = this.PC + 3;
                    break;
                ...
                case "6D": //Add with carry:  Adds contents of an address to the contents of the accumulator and keeps the result in the accumulator
                    ...
                    var addend = parseInt(_MemoryAccessor.read(this.getPhysicalAddress(this.lilEndianTranslator(), _CurrentPartition)), 16);
                    //add to accumulator
                    this.Acc = this.Acc + addend;
                    this.PC = this.PC + 3;
                    break;
                case "A2": //Load the X register with a constant
                    this.Xreg = parseInt(_MemoryAccessor.read(this.getPhysicalAddress(this.PC + 1, _CurrentPartition)), 16);
                    this.PC = this.PC + 2;
                    break;
                case "AE": //Load the X register from memory
                    //read content from memory address specified into xreg
                    this.Xreg = parseInt(_MemoryAccessor.read(this.getPhysicalAddress(this.lilEndianTranslator(), _CurrentPartition)), 16);
                    this.PC = this.PC + 3;
                    break;
                case "A0": //Load the Y register with a constant
                    this.Yreg = parseInt(_MemoryAccessor.read(this.getPhysicalAddress(this.PC + 1, _CurrentPartition)), 16);
                    this.PC = this.PC + 2;
                    break;
                case "AC": //Load the Y register from memory
                    this.Yreg = parseInt(_MemoryAccessor.read(this.getPhysicalAddress(this.lilEndianTranslator(), _CurrentPartition)), 16);
                    this.PC = this.PC + 3;
                    break;
                ...
                case "D0": //Branch n bytes if Z flag = 0
                    if (this.Zflag == 0) {
                        ...
                        var amt = parseInt(_MemoryAccessor.read(this.getPhysicalAddress(this.PC + 1, _CurrentPartition)), 16);
                ...
                case "FF": //System Call: #$01 in X reg = print the integer stored in the Y register. #$02 in X reg = print the 00-terminated string stored at the address in the Y register.
                    ...
                    if (this.Xreg == 1) {
                        ...
                        var addy = this.Yreg;
                        var print = "";
                        while (_MemoryAccessor.read(this.getPhysicalAddress(addy, _CurrentPartition)) != "00") {
                            //  var data = _MemoryAccessor.read(addy);
                            var decimal = parseInt(_MemoryAccessor.read(this.getPhysicalAddress(addy, _CurrentPartition)), 16);
        ...
        Cpu.prototype.lilEndianTranslator = function () {
            var memAdd = _MemoryAccessor.read(this.getPhysicalAddress(this.PC + 1, _CurrentPartition));
            memAdd = _MemoryAccessor.read(this.getPhysicalAddress(this.PC + 2, _CurrentPartition)) + memAdd;
